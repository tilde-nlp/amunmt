...
class SlowLSTM: public Cell {
  public:
    ...
    virtual void GetNextState(CellState& NextState,
      ...
      std::cerr << "O_=" << DebugRows(O_, 1, "O_=>") << std::endl;
      std::cerr << "F_=" << DebugRows(F_, 1, "F_=>") << std::endl;
      std::cerr << "I_=" << DebugRows(I_, 1, "I_=>") << std::endl;
      // compute the input
      ...
      std::cerr << "Output=" << mblas::DebugRows(O_, 1, "Output=>") << std::endl;
